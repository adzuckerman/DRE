({
  organizeData: function(timeDataByTask, component) {
    ...
    for (var taskId in timeDataByTask) {
      ...
      taskWithTimeItems.forEach(function(timeItem) {
        timeItem.TASKRAY__Hours__c =
          Math.round(timeItem.TASKRAY__Hours__c * 100) / 100;
    ...
    var allDaysSums = [
      mondaySum.toFixed(2),
      tuesdaySum.toFixed(2),
      wednesdaySum.toFixed(2),
      thursdaySum.toFixed(2),
      fridaySum.toFixed(2),
      saturdaySum.toFixed(2),
      sundaySum.toFixed(2)
    ];
    ...
    dayEntries.forEach(function(dayEntry) {
      var dayEntry = dayEntry.sort(function(entryA, entryB) {
        var entryADate =
          typeof entryA === 'undefined' &&
          typeof entryA.TASKRAY__Date__c === 'undefined'
            ? '0'
            : entryA.TASKRAY__Date__c;
        var entryBDate =
          typeof entryB === 'undefined' &&
          typeof entryB.TASKRAY__Date__c === 'undefined'
            ? '0'
            : entryB.TASKRAY__Date__c;
  ...
  formatDate: function(dateObj) {
    ...
    return $A.localizationService.formatDate(
      dateObj,
      dateFormat,
      userLocaleLang
    );
  },
  applyBillableDefault: function(component, event, helper) {
    var action = component.get('c.getTimeComponentBillableDefault');
    action.setParams({});
    action.setCallback(this, function(response) {
      var state = response.getState();
      if (state === 'SUCCESS') {
        component.set('v.billable', response.getReturnValue());
      } else {
      }
    });
    $A.enqueueAction(action);
  },
  applyTypeFieldInfo: function(component, event, helper) {
    var action = component.get('c.getTimeTypeVisibleOverride');
    action.setParams({});
    action.setCallback(this, function(response) {
      var state = response.getState();
      if (state === 'SUCCESS') {
        var typeFieldVisible = response.getReturnValue();
        console.log(typeFieldVisible);
        component.set('v.showTypeField', typeFieldVisible);
        if (typeFieldVisible) {
          helper.fetchTimeTypeFieldOptions(component, event, helper);
        }
      } else {
      }
    });
  ...
  fetchTimeTypeFieldOptions: function(component, event, helper) {
    var action = component.get('c.getTimeTypeFieldOptions');
    action.setParams({});
    action.setCallback(this, function(response) {
      var state = response.getState();
      if (state === 'SUCCESS') {
        console.log(response.getReturnValue());
        component.set('v.typeOptions', JSON.parse(response.getReturnValue()));
      } else {
      }
    });
    $A.enqueueAction(action);
